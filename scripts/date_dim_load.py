...
def generate_date_data(start_date, end_date):
    ...
    while current_date <= end_date:
        day_of_week = current_date.weekday() + 1  # Devuelve 0-6 (0=Lunes, 6=Domingo)
        ...
        date_data.append({
            ...
            'day_name': current_date.strftime('%A'),  # Nombre del día (Lunes, Martes...)
            'week': current_date.isocalendar()[1],  # Semana del año
            'month': current_date.month,
            'month_name': current_date.strftime('%B'),  # Nombre del mes (Enero, Febrero...)
...
def insert_date_data(date_data):
    for date_record in date_data:
        cursor.execute("""
            INSERT INTO date_dim (date, day, day_of_week, day_name, week, month, month_name, quarter, year, is_weekend, created_at)
