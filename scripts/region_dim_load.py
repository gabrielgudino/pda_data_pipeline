...
def insert_regions_into_region_dim(regions):
    for region, country in regions:
        ...
        if country_result is not None:
            ...
            cursor.execute("SELECT region_id FROM region_dim WHERE region_name = %s AND country_id = %s", (region, country_id))
            region_result = cursor.fetchone()

            # Si la región no existe, insertarla
            if region_result is None:
                cursor.execute("INSERT INTO region_dim (region_name, country_id) VALUES (%s, %s)", (region, country_id))
